...
export async function POST(request: NextRequest){
    try {
        ...
        // means previous vote doesnt exist or vote status changes
        if(response.documents[0]?.voteStatus !== voteStatus){
           ...
           // if vote is already present
           if(response.documents[0]){
            await users.updatePrefs<UserPrefs>(
                QuestionOrAnswer.authorId, {
                    reputation:
                    // it meaNS prev vote was "upvoted" and new vote is "downvoted" so we have to decrease the reputation
                ...
                await users.updatePrefs<UserPrefs>(
                    QuestionOrAnswer.authorId, {
                       reputation:
                       // it meaNS prev vote was "upvoted" and new vote is "downvoted" so we have to decrease the reputation
                       (response.documents[0]as any).voteStatus === "upvoted" ? Number(authorPrefs.reputation) + 1
                       : Number(authorPrefs.reputation) - 1
                    }
                )
           }
